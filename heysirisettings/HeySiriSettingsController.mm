...
@interface HeySiriSettingsController : PSListController
{
	UILabel* _label;
	UILabel* underLabel;
}
...
__strong static HeySiriSettingsController* HeySiriSettingsControllerC = nil;
static void settingsChangedHeySiriByToogle(CFNotificationCenterRef center, void *observer, CFStringRef name, const void *object, CFDictionaryRef userInfo)
{
	if (HeySiriSettingsControllerC != nil) {
		[HeySiriSettingsControllerC reloadSpecifiers];
	}
}

@implementation HeySiriSettingsController
- (id)init
{
	self = [super init];
	HeySiriSettingsControllerC = self;
	CFNotificationCenterRemoveEveryObserver(CFNotificationCenterGetDarwinNotifyCenter(), "com.julioverne.heysiri/SettingsChanged/Toogle");
	CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(), "com.julioverne.heysiri/SettingsChanged/Toogle", settingsChangedHeySiriByToogle, CFSTR("com.julioverne.heysiri/SettingsChanged/Toogle"), NULL, CFNotificationSuspensionBehaviorCoalesce);
    return self;
}
...
- (void)dealloc
{
	HeySiriSettingsControllerC = nil;
}
- (id)specifiers {
	if (!_specifiers) {
		...
		[spec setProperty:@"Requires Hey Siri Feature Enabled." forKey:@"footerText"];
		...
		if (access("/usr/lib/libactivator.dylib", F_OK) == 0) {
			...
			[spec setProperty:@"com.julioverne.heysiri" forKey:@"activatorListener"];
        ...
        [specifiers addObject:spec];
		spec = [PSSpecifier emptyGroupSpecifier];
        [spec setProperty:@"HeySiri © 2017" forKey:@"footerText"];
        [specifiers addObject:spec];
...
- (void)love
{
	...
	[twitter setInitialText:@"#HeySiri by @ijulioverne is cool!"];
	if (twitter != nil) {
...
- (void)HeaderCell
{
	@autoreleasepool {
		...
		[underLabel setText:@"Hey Siri While Not Charging"];
